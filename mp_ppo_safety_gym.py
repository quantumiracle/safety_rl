...
MODEL_PATH = f'./data/mp_PPO_continuous_{ENV_NAME}'
LOG_PATH = './data/'
...
def main():
    ...
    if args.train:
        ...
        costs_queue=mp.Queue()  # used for get costs from all processes and plot the curve
        lengths_queue=mp.Queue()  # used for get episode lengths from all processes and plot the curve
        queues = [rewards_queue, costs_queue, lengths_queue]
        processes=[]
        rewards=[]
        ...
        for i in range(NUM_WORKERS):
            if i == 0:  # one evaluate process
                ...
                process = Process(target=worker, args=(i, ppo, queues, False))  # the args contain shared and not shared
        ...
        while True:  # keep geting the episode reward from the queue
            ...
            if r is not None and c is not None:
                log['running_reward'].append(r)
                log['running_cost'].append(c)
                log['time_step'].append(cnt)
            if l is not None:
                log['avg_length'].append(l)
            else:
                break

            if cnt % LOG_INTERVAL == 0:
                json.dump(log, open(LOG_PATH + f"{ENV_NAME}_mp_ppo.json", 'w'))
                avg_length = np.mean(log['avg_length'][-LOG_INTERVAL:])
                running_reward = np.mean(log['running_reward'][-LOG_INTERVAL:])
                running_cost = np.mean(log['running_cost'][-LOG_INTERVAL:])
                
                print('Episodes {} \t Avg length: {} \t Avg reward: {%.3f} \t Avg cost: {}'\
