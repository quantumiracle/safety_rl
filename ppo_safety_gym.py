...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, action_std, hidden_dim = 128):
...
class PPO:
    ...
    def update(self, memory):
        ...
        old_states = torch.squeeze(torch.stack(memory.states).float().to(device)).detach()
        old_actions = torch.squeeze(torch.stack(memory.actions).float().to(device)).detach()
        old_logprobs = torch.squeeze(torch.stack(memory.logprobs)).float().to(device).detach()
